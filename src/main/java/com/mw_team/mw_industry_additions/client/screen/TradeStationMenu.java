...
public class TradeStationMenu extends AbstractContainerMenu {
    private final TradeStationEntity blockEntity;
    private final Level level;
    private final ContainerData data;

    public TradeStationMenu(int pContainerId, Inventory inv, FriendlyByteBuf extraData) {
        this(pContainerId, inv, inv.player.level.getBlockEntity(extraData.readBlockPos()), new SimpleContainerData(2));
    }

    public TradeStationMenu(int pContainerId, Inventory inv, BlockEntity entity, ContainerData data) {
        super(ModMenuTypes.trader_menu.get(), pContainerId);
        checkContainerSize(inv, 4);
        blockEntity = ((TradeStationEntity) entity);
        ...
        this.blockEntity.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY).ifPresent(handler -> {
            ...
            this.addSlot(new BasicModResultSlot(handler, 3, 80, 60));
        });
    ...
    public boolean stillValid(Player pPlayer) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()),
                pPlayer, ModBlocks.TRADE_STATION.get());
    }
